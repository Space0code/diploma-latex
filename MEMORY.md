...
- 2026-08-04 Enačba B.4 za učljive predznačene uteži je preverjena proti kodi in končnim konfiguracijam. Uporabljena je normalizacija po absolutni vsoti vhodnih uteži ciljnega vozlišča, ločeno znotraj vsake relacije, z $\epsilon=10^{-8}$. Pri modelu `HeteroGCNMLPWeights` plast `GCNConv` te uteži uporabi neposredno z `normalize=False` in `add_self_loops=False`; lasten signal vozlišča se ohranja prek rezidualne povezave. Negativna utež obrne predznak sporočila, ne pomeni zgolj njegove oslabitve.
