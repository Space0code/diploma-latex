...
Negrafovski osnovni modeli prejmejo agregirano tabelarično predstavitev posameznega časovnega okna, zamrznjeni prednaučeni predstavitveni modeli dobijo časovne vrste izbranih signalov, grafovski modeli pa delujejo na grafu, ki predstavlja časovno okno meritev.
...
\end{equation}
Pri tem je $g$ naučena funkcija, ki predstavitvi vozlišča priredi skalarno oceno pomembnosti.
...
Relacijski izhodi $\mathbf{Z}_r^{(k)}$ se izračunajo kot v enačbi~\eqref{eq:model-heterogcn-mean}, njihovo naučeno fuzijo pa zapišemo kot
\begin{equation}
    \mathbf{Z}^{(k)} =
...
Kot prikazano v diagramu~\ref{fig:diagram-heterogcn-mlp}, je sprememba glede na \ModelName{HeteroGCN-mean} v zamenjavi ročno določene fuzije z naučeno fuzijo relacijskih predstavitev.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{\detokenize{HeteroGCN-MLP final.png}}
    \caption[Diagram arhitekture \ModelName{HeteroGCN-MLP}.]{Diagram arhitekture \ModelName{HeteroGCN-MLP}. Model relacijske predstavitve najprej konkatenira, nato pa jih združi z naučeno fuzijo prek dvoslojnega perceptrona.}
...
Slika~\ref{fig:diagram-heterogcn-mlp-w} prikazuje dodano vejo za izračun naučenih uteži povezav.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{\detokenize{HeteroGCN-MLP-w final.png}}
    \caption[Diagram arhitekture \ModelName{HeteroGCN-MLP-w}.]{Diagram arhitekture \ModelName{HeteroGCN-MLP-w}. Model poleg naučene fuzije relacij uporablja še značilke povezav, iz katerih z relacijsko odvisnimi perceptroni izračuna naučene predznačene uteži povezav.}
...
Pri heterogenem modelu plasti \texttt{GCNConv} in \texttt{GraphConv} uporabljata naučene skalarne uteži povezav, \texttt{GATConv} pa značilke povezav vključi neposredno v izračun pozornosti in ločenih skalarnih uteži ne uporablja.
Plast \texttt{GINEConv} je v implementaciji razširjena kot \texttt{WeightedGINEConv}, ki poleg značilk povezav uporabi tudi naučene skalarne uteži.
