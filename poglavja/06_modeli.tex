...
\subsection{Posredovanje sporočil in skupni cevovod}
...
V dodatnem poskusu smo ga primerjali s plastmi \texttt{GATConv}~\cite{velickovic2018gat}, \texttt{GraphConv}~\cite{morris2019weisfeiler} ter \texttt{GINConv} oziroma \texttt{GINEConv}~\cite{xu2019powerful,hu2020strategies}.
\texttt{GATConv} z mehanizmom pozornosti nauči uteži prispevkov sosednjih vozlišč, \texttt{GraphConv} pa ločeno preslika predstavitev ciljnega vozlišča in agregirane predstavitve njegovih sosedov.
\texttt{GINConv} agregirano predstavitev obdela z večslojnim perceptronom, \texttt{GINEConv} pa isti pristop razširi z značilkami povezav.
Pri osnovnem \ModelName{GCN} smo preizkusili \texttt{GINConv}, pri modelu \ModelName{HeteroGCN-MLP-w} pa \texttt{GINEConv}.
Pri heterogenem modelu plasti \texttt{GCNConv} in \texttt{GraphConv} uporabljata učljive skalarne uteži povezav, \texttt{GATConv} pa značilke povezav vključi neposredno v izračun pozornosti in ločenih skalarnih uteži ne uporablja.
Plast \texttt{GINEConv} je v implementaciji razširjena kot \texttt{WeightedGINEConv}, ki poleg značilk povezav uporabi tudi učljive skalarne uteži.

Poskus smo izvedli na binarni nalogi valence z množico signalov \emph{pogled in zenici}.
Razlike med operatorji so bile razmeroma majhne, \texttt{GCNConv} pa je dosegel primerljive in konsistentne rezultate tako pri osnovnem homogenem modelu kot pri uteženi heterogeni arhitekturi.
