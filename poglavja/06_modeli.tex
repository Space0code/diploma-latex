...
\begin{table}[t]
    ...
    \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.15\textwidth} >{\raggedright\arraybackslash}p{0.25\textwidth} >{\raggedright\arraybackslash}p{0.22\textwidth} >{\raggedright\arraybackslash}X@{}}
        ...
        \cmidrule(lr){1-4}
        SVM & \multirow{3}{=}{klasični modeli strojnega učenja} & \multirow{3}{=}{agregirane značilke okna} & \multirow{3}{=}{klasični model strojnega učenja} \\
        LightGBM & & & \\
        MLP & & & \\
        \cmidrule(lr){1-4}
        \GazeMAE{} & \multirow{2}{=}{zamrznjeni prednaučeni predstavitveni modeli} & koordinati pogleda $(x, y)$ & prednaučena predstavitev za množico \emph{samo pogled} \\
        MOMENT & & časovne vrste izbranih signalov & prednaučena predstavitev za množice s signali zenic oziroma več signali \\
        \cmidrule(lr){1-4}
        GCN & \multirow{4}{=}{grafovski modeli} & homogeni graf okna & osnovni grafovski model \\
        HeteroGCN-mean & & \multirow{2}{=}{heterogeni graf okna} & ločena obravnava relacij s preprosto fuzijo \\
...
Ker je SVM občutljiv na velikostni red vhodnih značilk, agregirane značilke pred učenjem standardiziramo.
...
družino modelov \GazeMAE{}/MOMENT.
Pri množici \emph{samo pogled} ta vrstica pomeni uporabo \GazeMAE{}, pri množici
\emph{samo zenici} uporabo MOMENT, pri množicah \emph{pogled in zenici} ter
\emph{vsi signali} pa združeno predstavitev obeh kodirnikov.
...
\begin{table}[t]
    ...
    \small
    \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.20\textwidth} >{\centering\arraybackslash}p{0.105\textwidth} >{\centering\arraybackslash}p{0.13\textwidth} >{\centering\arraybackslash}p{0.11\textwidth} >{\raggedright\arraybackslash}X@{}}
        \toprule
...
\END_NOTE{Zamenjaj to enačbo z diagramom, ko ga imaš. Z diagramom lažje prikažeš že samo razliko med tem in naslednjim modelom in imaš potem majhen diagram v vsaki subsekciji in obkrožiš razlike pri vsaki nadgradnji}

\subsection{HeteroGCN-mean}
\label{subsec:model-heterogcn-mean}

\emph{HeteroGCN-mean} dobimo tako, da \emph{GCN} nadgradimo s heterogeno grafovsko predstavitvijo.
...
\emph{HeteroGCN-MLP} dobimo tako, da \emph{HeteroGCN-mean} nadgradimo z naučeno fuzijo relacijskih predstavitev.
...
\emph{HeteroGCN-MLP-w} dobimo tako, da \emph{HeteroGCN-MLP} nadgradimo z naučenimi utežmi povezav.
Za izračun uteži povezav ima model tri večslojne perceptrone: $f_w^{\mathrm{čas}}$, $f_w^{\mathrm{prostor}}$ in $f_w^{\mathrm{fiksacija}}$.
...
Časovne povezave naprej in nazaj si delijo isto funkcijo $f_w^{\mathrm{čas}}$, smer povezave pa je podana kot dodatna značilka povezave $\rho_{ij}$.
