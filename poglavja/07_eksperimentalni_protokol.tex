...
Cilj glavne primerjave je odgovoriti na raziskovalno vprašanje iz razdelka~\ref{subsec:raziskovalna-vprasanja}: kakšna je klasifikacijska uspešnost grafovskih modelov v primerjavi z negrafovskimi pristopi pri isti ciljni nalogi in istih osnovnih podatkovnih oknih.
...
Vse štiri množice iz tabele~\ref{tab:mnozice-signalov} obravnavamo kot enakovreden del primerjave, saj nas zanima tudi, kako se klasifikacijska uspešnost spreminja z razpoložljivostjo različnih signalov sledilnika pogleda.
...
Iz učnega dela izberemo še enega subjekta za validacijsko množico, preostali subjekti pa sestavljajo učno množico.
...
Učenje lahko traja največ 350 epoh, potrpežljivost zgodnjega ustavljanja je 20 epoh, velikost mini-serije pa 256.
Glavne nastavitve nevronskih in grafovskih modelov so povzete v tabeli~\ref{tab:nastavitve-ucenja}, celoten seznam hiperparametrov pa je v dodatku~\ref{app:hiperparametri}.

\begin{table}[t]
    \centering
    \caption{Glavne nastavitve učenja za nevronske in grafovske modele v končnih eksperimentih.}
    \label{tab:nastavitve-ucenja}
    \small
    \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.34\textwidth} >{\raggedright\arraybackslash}X@{}}
        \toprule
        Nastavitev & Vrednost \\
        \midrule
        Največje število epoh & 350 \\
        Zgodnje ustavljanje & validacijska izguba, potrpežljivost 20 epoh \\
        Velikost mini-serije & 256 \\
        Grafovski operator & \texttt{GCNConv} \\
        Učna stopnja grafovskih modelov & $5 \cdot 10^{-5}$ \\
        Število grafovskih plasti & \texttt{num\_layers=2} \\
        Velikost skrite predstavitve & \texttt{hidden\_channels=64} \\
        Časovne povezave & \texttt{kt=1} \\
        Prostorske povezave & \texttt{ks=1} \\
        Razširjene povezave znotraj fiksacije & \texttt{kf=3} \\
        \bottomrule
    \end{tabularx}
\end{table}

Nastavitve grafovskih modelov izberemo na podlagi predhodnega kompaktnega mrežnega iskanja.
To iskanje je bilo izvedeno na 3-razredni valenci in ga poročamo samo kot pomožni poskus za izbiro stabilnih nastavitev.
...
Točnost uporabimo kot osnovno metriko, ker ima glavna naloga dva enakovredna in globalno skoraj uravnotežena razreda.

Makro F1 uporabimo kot dopolnilno glavno metriko.
Čeprav sta razreda globalno skoraj uravnotežena, so lahko posamezne testne razdelitve precej neuravnotežene.
...
Te diagnostike ne poročamo v poglavju~\ref{sec:rezultati}, je pa podana v dodatku~\ref{app:diagnostika-prekomerno-glajenje}.
